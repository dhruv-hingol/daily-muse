...
export default async function handler(
    ...
    try {
        if (!process.env.OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY.includes('your_api_key_here')) {
        ...
        if (type === 'image') {
            ...
            const promptCompletion = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are an expert image prompt engineer. Convert the user's word or simple topic into a highly detailed, artistic, and visually stunning image prompt for DALL-E 3. Focus on lighting, style, and composition. Respond ONLY with the refined prompt."
                    },
                    {
                        role: "user",
                        content: word
            ...
            const refinedPrompt = promptCompletion.choices[0].message.content?.trim() || word;

            // Attempt image generation
            try {
                // This model is a chat model that returns an image URL in its content.
                // We set a low max_tokens to avoid credit-limit issues on OpenRouter.
                const imageCompletion = await openai.chat.completions.create({
                    ...
                    max_tokens: 2000, // Increased to accommodate reasoning tokens + URL
                });
        ...
        const completion = await openai.chat.completions.create({
            model: "openai/gpt-5.2", // Using a more standard model name
            messages: [
                {
                    role: "system",
                    content: systemPrompts[type as Exclude<ContentType, 'image'>] || systemPrompts.quote
                },
                {
                    role: "user",
                    content: userPrompts[type as Exclude<ContentType, 'image'>] || userPrompts.quote
                }
            ],
            temperature: 0.8,
            max_tokens: 250,
        ...
        try {
            ...
            if (type === 'joke') {
                responseData = { joke: text, punchline: "", explanation: "A witty response." };
            } else if (type === 'news') {
                responseData = { headline: "News Update", summary: text, context: "" };
            } else {
                responseData = { quote: text, author: "Unknown", interpretation: "Insightful words." };
            }
        }
